...
public abstract class TinyProtocol {
	...
	private static final AtomicInteger ID_RAW = new AtomicInteger(0);
	...
	public TinyProtocol(final Plugin plugin) {
		...
		this.rawHandlerName = getRawHandlerName();
	...
	// (DJ)
	protected String getRawHandlerName() {
		return "tiny-" + plugin.getName() + "-raw-" + ID_RAW.incrementAndGet();
	}
	...
	private PacketInterceptor injectChannelInternal(Channel channel) {
		try {
			...
			// (DJ) Inject raw packet interceptor before packet decoder
			RawPacketInterceptor rawInterceptor = (RawPacketInterceptor) channel.pipeline().get(rawHandlerName);

			if (rawInterceptor == null) {
				rawInterceptor = new RawPacketInterceptor();
				channel.pipeline().addBefore("decoder", rawHandlerName, rawInterceptor);
			}
	...
	public void uninjectChannel(final Channel channel) {
		...
		channel.eventLoop().execute(new Runnable() {
			...
			public void run() {
				...
				channel.pipeline().remove(rawHandlerName);
			}
	...
	// (DJ) custom packet interceptor to grab packet bytes before decoded
	// (DJ) injected in injectChannelInternal()
	private final class RawPacketInterceptor extends ChannelInboundHandlerAdapter {
		@Override
		public void channelRead(ChannelHandlerContext ctx, Object o) throws Exception {
			boolean allow = onRawPacketIn(ctx.channel(), o);
			if (allow) {
